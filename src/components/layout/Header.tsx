...
export default function Header() {
  ...
  const isCategorySelectedAtLevel = (categoryId: string, level: number): boolean => {
    return selectionPath[level]?.id === categoryId;
  };

  const buildPanels = (rootCategory: CategoryTreeNode): PanelData[] => {
  ...
  const renderPanel = (panel: PanelData, panelIndex: number): React.ReactNode => {
                ...
                return (
                  <div key={category.id} className="relative">
                    ...
                    >
                      ...
                      <span className={`absolute bottom-1 left-1/2 -translate-x-1/2 h-0.5 bg-gradient-to-r from-orange-400 to-orange-600 rounded-full transition-all duration-300 ease-out ${
              ...
              <AnimatePresence>
                {isLangMenuOpen && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 10 }}
                    className="absolute top-full right-0 mt-2 w-40 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden z-[100]"
                  >
                    {languages.map((lang) => (
                      <button
                        key={lang.code}
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
              ...
              className="lg:hidden mt-4 pb-4 border-t border-gray-100 max-h-[70vh] overflow-hidden"
            >
                ...
                <AnimatePresence mode="wait">
                  {mobileDrilldownStack.length === 0 ? (
                    <motion.nav
                      ...
                      exit={{ opacity: 0, x: -20 }}
                      transition={{ duration: 0.25, ease: [0.4, 0, 0.2, 1] }}
                      className="flex flex-col pt-4 max-h-[60vh] overflow-y-auto"
                    >
                      <div className="pt-2">
                          ...
                          if (hasChildren) {
                            return (
                              <motion.button
                                ...
                                className={`group w-full flex items-center justify-between mx-4 px-4 py-3.5 rounded-xl transition-all duration-300 active:scale-[0.98] ${
                                  isActive
                              ...
                              >
                                ...
                                {isActive && (
                                  <span className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-gradient-to-b from-orange-400 to-orange-600 rounded-r-full" />
                                )}
                              </motion.button>
                            );
                          }
                          ...
                          return (
                            ...
                            >
                              <Link
                                to={`/products?category=${category.id}`}
                                onClick={handleCategoryLinkClick}
                                className={`group relative flex items-center gap-3 mx-4 px-4 py-3.5 rounded-xl transition-all duration-300 active:scale-[0.98] ${
                                  isActive
                                    ? 'bg-gradient-to-r from-orange-50 to-orange-100/50'
                                    : 'hover:bg-gray-50'
                                }`}
                      ...
                      >
                        <Link
                          to="/products"
                          onClick={handleCategoryLinkClick}
                          className="group flex items-center justify-between mx-4 mt-4 px-4 py-3.5 rounded-xl bg-gradient-to-r from-orange-500 to-orange-600 text-white transition-all duration-300 hover:shadow-lg hover:shadow-orange-500/25 active:scale-[0.98]"
                        >
                          <span className="text-[15px] font-semibold tracking-wide">{t.home.viewAll}</span>
                          <ChevronRight className="w-5 h-5 transition-transform group-hover:translate-x-1" />
                        </Link>
                      </motion.div>

                      <div className="mx-4 px-4 py-4 mt-4 border-t border-gray-100 flex items-center gap-3">
                        <span className="w-9 h-9 rounded-xl bg-gradient-to-br from-orange-100 to-orange-50 flex items-center justify-center">
                          <Phone className="w-5 h-5 text-orange-500" />
                        </span>
                        <span className="text-[15px] font-semibold text-gray-700 tracking-wide">{t.nav.phone}</span>
                      </div>
                    </motion.nav>
                  ) : (
                    <motion.div
                    ...
                    >
                      <button
                        ...
                        className="group w-full flex items-center gap-3 px-6 py-4 font-semibold text-orange-500 bg-gradient-to-r from-orange-50 to-transparent border-b border-orange-100 transition-all duration-300 active:scale-[0.99]"
                      >
                      ...
                      <Link
                        ...
                        onClick={handleCategoryLinkClick}
                        className="group flex items-center gap-2 px-6 py-3.5 text-sm font-semibold text-orange-500 bg-orange-50/50 border-b border-gray-100 transition-all duration-300 hover:bg-orange-50"
                      >
                        {language === 'uz' ? 'Hammasini ko\'rish' : language === 'ru' ? 'Смотреть все' : 'View All'}
                          ...
                          if (hasChildren) {
                            return (
                              <motion.button
                                ...
                                onClick={() => handleMobileDrilldown(category)}
                                className="group w-full flex items-center justify-between px-6 py-3.5 transition-all duration-300 hover:bg-orange-50 active:scale-[0.99]"
                              >
                                <span className="flex items-center gap-3">
                                  {renderCategoryIcon(category, 'sm')}
                                  <span className="text-[15px] font-medium text-gray-700 group-hover:text-orange-500 transition-colors tracking-wide">
                                    {getLocalizedField(category, 'name')}
                                  </span>
                                </span>
                          ...
                          return (
                            ...
                            >
                              <Link
                                to={`/products?category=${category.id}`}
                                onClick={handleCategoryLinkClick}
                                className="group flex items-center gap-3 px-6 py-3.5 transition-all duration-300 hover:bg-orange-50 active:scale-[0.99]"
                              >
                                {renderCategoryIcon(category, 'sm')}
                                <span className="text-[15px] font-medium text-gray-700 group-hover:text-orange-500 transition-colors tracking-wide">
                                  {getLocalizedField(category, 'name')}
